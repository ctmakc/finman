...
const ReportsModule = {
  ...
  renderHistory() {
    ...
    if (this.reports.length === 0) {
      container.innerHTML = '<p class="text-secondary">Нет созданных отчётов</p>';
      return;
    }

    container.innerHTML = `<table class="table">
      <thead><tr><th>Название</th><th>Тип</th><th>Период</th><th>Дата</th><th></th></tr></thead>
      <tbody>
        ${this.reports.map(r => `
          <tr>
            <td>${r.title}</td>
  ...
  showGenerateModal(type) {
    ...
    document.getElementById('generate-modal-title').textContent = typeInfo ? typeInfo.name : 'Генерация отчёта';
  ...
  showReportResult(data, type) {
    ...
    if (type === 'monthly' || type === 'yearly') {
      html += `
        <div class="report-summary">
          ...
          <div class="stats-grid">
            <div class="stat-card positive"><div class="stat-value">${(data.summary?.income || 0).toLocaleString()} ₴</div><div class="stat-label">Доходы</div></div>
            <div class="stat-card negative"><div class="stat-value">${(data.summary?.expenses || 0).toLocaleString()} ₴</div><div class="stat-label">Расходы</div></div>
            <div class="stat-card"><div class="stat-value">${(data.summary?.balance || data.summary?.savings || 0).toLocaleString()} ₴</div><div class="stat-label">Баланс</div></div>
          </div>
        </div>
      `;
      if (data.byCategory) {
        html += '<h3>По категориям</h3><div class="categories-list">';
        data.byCategory.forEach(c => {
          html += `<div class="category-row"><span>${c.category || 'Без категории'}</span><span>${c.total.toLocaleString()} ₴</span></div>`;
        });
        html += '</div>';
      }
      if (data.monthly) {
        html += '<h3>По месяцам</h3><table class="table"><thead><tr><th>Месяц</th><th>Доходы</th><th>Расходы</th></tr></thead><tbody>';
      ...
      html += '<h3>Расходы по категориям</h3><div class="categories-list">';
      ...
      html += `<h3>Net Worth</h3><p>Текущий: <strong>${(data.current?.net_worth || 0).toLocaleString()} ₴</strong></p>`;
      html += `<p>Изменение: <strong class="${data.change >= 0 ? 'positive' : 'negative'}">${(data.change || 0).toLocaleString()} ₴ (${data.changePercent || 0}%)</strong></p>`;
    } else if (type === 'tax') {
      html += `
        <h3>Налоговый отчёт</h3>
        <p>Общий доход: <strong>${(data.totalIncome || 0).toLocaleString()} ₴</strong></p>
        <h4>Оценка налогов:</h4>
        <ul>
          <li>5% (3 группа ФОП): ${(data.estimatedTax5 || 0).toLocaleString()} ₴</li>
          <li>18% НДФЛ: ${(data.estimatedTax18 || 0).toLocaleString()} ₴</li>
          <li>ЕСВ: ${(data.estimatedESV || 0).toLocaleString()} ₴</li>
        </ul>
      `;
    } else {
  ...
  getPage() {
    return `
      <div class="reports-page">
        <div class="page-header"><h1>📊 Отчёты</h1></div>
        <div class="card"><h2>Создать отчёт</h2><div id="report-types" class="report-types-grid"></div></div>
        <div class="card"><h2>История отчётов</h2><div id="reports-history"></div></div>
      </div>
      <div class="modal" id="generate-modal">
        <div class="modal-content">
          <div class="modal-header"><h2 id="generate-modal-title">Генерация отчёта</h2><button class="modal-close" onclick="document.getElementById('generate-modal').classList.remove('active')">&times;</button></div>
      ...
      <div class="modal" id="result-modal">
        <div class="modal-content modal-lg">
          <div class="modal-header"><h2>Результат отчёта</h2><button class="modal-close" onclick="document.getElementById('result-modal').classList.remove('active')">&times;</button></div>
